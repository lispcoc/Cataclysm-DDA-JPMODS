...
local mon_tbl_special = { "mon_kreck",
						  "mon_mi_go",
						  "mon_mi_go",
						  "mon_gozu",
						  "mon_kreck",
...
function get_mon_id(waves, allow_special, quality_freq)
	--特殊襲撃時は問答無用でspecialなモンスター(初期設定：NETHERモンスター)を出現させる
	if allow_special == 1 and
	   waves % 13 == 0 then
		if game.one_in(666) then						--"1/666の確率でヤベーヤツ"
			return mon_tbl_special[#mon_tbl_special]
		end
		if game.one_in(3) then							--"1/3の確率でショゴス・フレーミングアイを含む"
			return mon_tbl_special[math.random(1,( #mon_tbl_special - 1 ))]
		end												--"それ以外ならヤベーの3種を除いてランダム"
		return mon_tbl_special[math.random(1,( #mon_tbl_special - 3 ))]
	end
